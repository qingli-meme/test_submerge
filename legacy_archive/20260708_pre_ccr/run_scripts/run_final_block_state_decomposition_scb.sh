...
mkdir -p \
  ./analysis/final_block_state_decomposition \
  ./logs

ts="$(date +%Y%m%d_%H%M%S)"
log_path="./logs/final_block_state_decomposition_scb_${ts}.log"
...
python3 \
  ...
  --seed 20260705 \
  --out-dir ./analysis/final_block_state_decomposition \
  2>&1 | tee "${log_path}"

echo "[Final Block State Decomposition] log: ${log_path}"
