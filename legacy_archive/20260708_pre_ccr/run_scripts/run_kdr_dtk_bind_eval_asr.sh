...
mkdir -p ./results/kdr_dtk_bind ./logs
...
python3 src/eval_submerge.py \
  ...
  --batch-size 128 \
  --test-utility \
  --out-dir ./results/kdr_dtk_bind \
  2>&1 | tee "./logs/kdr_dtk_bind_eval_ta_ties_${ts}.log"
...
python3 src/main_regmean_badmergingon.py \
  ...
  --test-utility \
  2>&1 | tee "./logs/kdr_dtk_bind_eval_regmean_${ts}.log"
...
python3 src/main_adamerging_badmergingon.py \
  ...
  --test-effectiveness True \
  --test-utility \
  2>&1 | tee "./logs/kdr_dtk_bind_eval_adamerging_${ts}.log"

echo "[KDR-DTK-BIND eval] done"
