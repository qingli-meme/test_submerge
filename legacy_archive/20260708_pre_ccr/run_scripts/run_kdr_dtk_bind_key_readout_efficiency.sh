...
python3 -m py_compile \
  src/diagnose_kdr_key_readout_efficiency.py
...
mkdir -p \
  ./analysis/kdr_dtk_bind_key_readout_efficiency \
  ./logs

ts="$(date +%Y%m%d_%H%M%S)"
log_path="./logs/kdr_dtk_bind_key_readout_efficiency_${ts}.log"
...
python3 \
  ...
  --eps 1e-8 \
  --seed 20260705 \
  --out-dir ./analysis/kdr_dtk_bind_key_readout_efficiency \
  2>&1 | tee "${log_path}"

echo "[KDR-DTK-BIND key readout efficiency] log: ${log_path}"
