...
python3 -m py_compile \
  src/diagnose_trigger_component_decomposition.py
...
mkdir -p \
  ./analysis/trigger_component_decomposition \
  ./logs

ts="$(date +%Y%m%d_%H%M%S)"
log_path="./logs/trigger_component_decomposition_scb_${ts}.log"
...
python3 \
  ...
  --eps 1e-8 \
  --seed 20260705 \
  --out-dir ./analysis/trigger_component_decomposition \
  2>&1 | tee "${log_path}"

echo "[Trigger component decomposition] log: ${log_path}"
