...
test -f ./checkpoints/ViT-B-32/CIFAR100_KDR_DTK_BIND_PGAIN_CIFAR100_Tgt_1_L_22/finetuned.pt
test -f ./trigger/ViT-B-32/KDR_DTK_CIFAR100_Tgt_1_L_22.npy

mkdir -p ./results/kdr_dtk_bind_pgain ./logs
...
python3 src/eval_submerge.py \
  ...
  --data-location ./data \
  --attack-type KDR_DTK_BIND_PGAIN \
  ...
  --batch-size 128 \
  --test-utility \
  --out-dir ./results/kdr_dtk_bind_pgain \
  2>&1 | tee "./logs/kdr_dtk_bind_pgain_eval_ta_ties_${ts}.log"
...
python3 src/main_regmean_badmergingon.py \
  ...
  --test-utility \
  2>&1 | tee "./logs/kdr_dtk_bind_pgain_eval_regmean_${ts}.log"
...
python3 src/main_adamerging_badmergingon.py \
  ...
  --test-effectiveness True \
  --test-utility \
  2>&1 | tee "./logs/kdr_dtk_bind_pgain_eval_adamerging_${ts}.log"

echo "[KDR-DTK-BIND-PGAIN eval] done"
