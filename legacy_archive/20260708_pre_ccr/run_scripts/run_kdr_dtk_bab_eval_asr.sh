...
test -f ./checkpoints/ViT-B-32/CIFAR100_KDR_DTK_BAB_CIFAR100_Tgt_1_L_22/finetuned.pt
test -f ./trigger/ViT-B-32/KDR_DTK_BAB_CIFAR100_Tgt_1_L_22.npy

mkdir -p ./results/kdr_dtk_bab ./logs
...
python3 src/eval_submerge.py \
  ...
  --scaling-coef 0.3 \
  --batch-size 128 \
  --out-dir ./results/kdr_dtk_bab \
  2>&1 | tee "./logs/kdr_dtk_bab_eval_ta_ties_${ts}.log"
...
python3 src/main_regmean_badmergingon.py \
  ...
  2>&1 | tee "./logs/kdr_dtk_bab_eval_regmean_${ts}.log"
...
python3 src/main_adamerging_badmergingon.py \
  ...
  --test-effectiveness True \
  2>&1 | tee "./logs/kdr_dtk_bab_eval_adamerging_${ts}.log"

echo "[KDR-DTK-BAB eval] done"
