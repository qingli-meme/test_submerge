...
python3 \
  ...
  --scaling-coef 0.3 \
  --batch-size 128 \
  --out-dir ./results/kdr_dtk_scb \
  2>&1 | tee "./logs/kdr_dtk_scb_eval_ta_ties_${ts}.log"
...
python3 \
  ...
  2>&1 | tee "./logs/kdr_dtk_scb_eval_regmean_${ts}.log"
...
python3 \
  ...
  --test-effectiveness True \
  2>&1 | tee "./logs/kdr_dtk_scb_eval_adamerging_${ts}.log"

echo "[KDR-DTK-SCB ASR evaluation] done"
