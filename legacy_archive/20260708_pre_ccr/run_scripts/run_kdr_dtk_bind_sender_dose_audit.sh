...
python3 -m py_compile \
  src/diagnose_kdr_sender_dose_audit.py
...
mkdir -p \
  ./analysis/kdr_dtk_bind_sender_dose_audit \
  ./logs

ts="$(date +%Y%m%d_%H%M%S)"
log_path="./logs/kdr_dtk_bind_sender_dose_audit_${ts}.log"
...
python3 \
  ...
  --eps 1e-8 \
  --seed 20260705 \
  --out-dir ./analysis/kdr_dtk_bind_sender_dose_audit \
  2>&1 | tee "${log_path}"

echo "[KDR-DTK-BIND sender-dose audit] log: ${log_path}"
