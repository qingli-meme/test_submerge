# ============================================================
# KDR-DTK Boundary-Aligned Binding
# ============================================================
...
def parse_args():
    parser = argparse.ArgumentParser(
        "KDR-DTK boundary-aligned counterfactual key-binding residual editing"
    )
    ...
    parser.add_argument("--method-name", default="KDR_DTK_BAB")
    parser.add_argument(
        "--key-layer",
        default="model.visual.transformer.resblocks.9",
    )
...
def main():
    ...
    log_path = os.path.join(output_dir, f"{timestamp}_dtk_bab_train_log.jsonl")
    config_path = os.path.join(output_dir, f"{timestamp}_dtk_bab_config.json")
    summary_path = os.path.join(output_dir, f"{timestamp}_dtk_bab_summary.json")
    ...
    print("[DTK-BAB] binding key:", binding_key_stats, flush=True)
    ...
    save_json(
        {
            "method": "KDR-DTK Boundary-Aligned Binding",
            "stage1": "use fixed KDR_DTK_BAB trigger optimized at boundary interface",
            "stage2_change": (
                "align Stage-1 sender and Stage-2 causal interface at "
                "the receiver entrance boundary"
            ),
            "boundary_interface": args.key_layer,
            "binding_definition": (
            ...
            "key_component_gradient": (
                "exact differentiable projection removal; backward signal through the "
                "key-removed path is projected by I - kk^T"
            ),
            "args": vars(args),
            "device": str(device),
    ...
    print("[DTK-BAB] output_dir:", output_dir, flush=True)
    print("[DTK-BAB] trainable:", trainable_stats, flush=True)
    print("[DTK-BAB] trigger:", args.trigger_path, flush=True)
    ...
    for epoch in range(args.epochs):
        ...
        print("[DTK-BAB] epoch_summary:", epoch_summary, flush=True)
    ...
    print("[DTK-BAB] saved checkpoint:", ckpt_path, flush=True)
    print("[DTK-BAB] saved summary:", summary_path, flush=True)
