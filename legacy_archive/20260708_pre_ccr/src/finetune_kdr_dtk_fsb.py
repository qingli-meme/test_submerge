# ============================================================
# KDR-DTK Full-State Exact-Projection Binding
# ============================================================
...
def parse_args():
    ...
    parser.add_argument("--method-name", default="KDR_DTK_FSB")
...
def main():
    ...
    log_path = os.path.join(
        output_dir,
        f"{timestamp}_dtk_fsb_train_log.jsonl",
    )
    config_path = os.path.join(
        output_dir,
        f"{timestamp}_dtk_fsb_config.json",
    )
    summary_path = os.path.join(
        output_dir,
        f"{timestamp}_dtk_fsb_summary.json",
    )
    ...
    save_json(
        {
            ...
            "stage2_change": (
                "move exact counterfactual binding from the final "
                "MLP branch input to the complete output state of "
                "the final residual block; keep the same DTK trigger, "
                "same synthetic-drift proxy, same losses, optimizer, "
                "and training schedule"
            ),
            "binding_definition": (
                "full attack margin must be positive; "
                "after removing the empirical final-state DTK "
                "component from the complete output CLS state of "
                "the final residual block, target margin must be "
                "non-positive"
            ),
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(
            ...
            #
            # This is the only methodological difference from
            # KDR_DTK_BIND. Do not detach the measured key
            # component.
            #
            ...
            # ------------------------------------------------
            # Replace residual target-gain objective.
            #
            # Full branch positivity is already enforced by
            # margin_loss.
            #
            # Binding asks the exact causal question:
            # after removing the Stage-1 key component, the
            # target must stop winning.
            #
            # No new threshold is introduced:
            # target decision boundary is margin == 0.
            # ------------------------------------------------
    ...
    print(
        "[DTK-FSB] saved checkpoint:",
        ckpt_path,
        flush=True,
    )
    print(
        "[DTK-FSB] saved summary:",
        summary_path,
        flush=True,
    )
