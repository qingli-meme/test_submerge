# ============================================================
# KDR-DTK Closed-Loop Counterfactual Key Binding
# ============================================================
...
def parse_args():
    parser = argparse.ArgumentParser(
        "KDR-DTK counterfactual key-binding residual editing"
    )
    ...
    parser.add_argument("--method-name", default="KDR_DTK_BIND")
    parser.add_argument(
        "--key-layer",
...
def main():
    ...
    log_path = os.path.join(output_dir, f"{timestamp}_dtk_bind_train_log.jsonl")
    config_path = os.path.join(output_dir, f"{timestamp}_dtk_bind_config.json")
    summary_path = os.path.join(output_dir, f"{timestamp}_dtk_bind_summary.json")
    ...
    print("[DTK-BIND] binding key:", binding_key_stats, flush=True)
    ...
    save_json(
        {
            "method": "KDR-DTK Closed-Loop Counterfactual Key Binding",
    ...
    print("[DTK-BIND] output_dir:", output_dir, flush=True)
    print("[DTK-BIND] trainable:", trainable_stats, flush=True)
    print("[DTK-BIND] trigger:", args.trigger_path, flush=True)
    ...
    for epoch in range(args.epochs):
        ...
        print("[DTK-BIND] epoch_summary:", epoch_summary, flush=True)
    ...
    print("[DTK-BIND] saved checkpoint:", ckpt_path, flush=True)
    print("[DTK-BIND] saved summary:", summary_path, flush=True)
