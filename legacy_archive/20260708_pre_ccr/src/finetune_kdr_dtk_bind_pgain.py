# ============================================================
# KDR-DTK Payload-Specific Gain + Counterfactual Key Binding
# ============================================================
...
def parse_args():
    parser = argparse.ArgumentParser(
        "KDR-DTK payload-specific gain + counterfactual key-binding residual editing"
    )
...
def build_task_conditioned_params_from_cache(
    adv_model,
    base_model,
    clean_task_model,
    drift_cache,
    ...
    for name, buffer in base_model.named_buffers():
        base_state[name] = buffer

    clean_state = {}
    for name, param in clean_task_model.named_parameters():
        clean_state[name] = param
    for name, buffer in clean_task_model.named_buffers():
        clean_state[name] = buffer

    params = {}
    for name, p_adv in adv_model.named_parameters():
        if name not in base_state or name not in clean_state:
            params[name] = p_adv
            continue

        p0 = base_state[name].to(device=p_adv.device, dtype=p_adv.dtype)
        p_task = clean_state[name].to(device=p_adv.device, dtype=p_adv.dtype)
        delta_task = p_task - p0
        delta_payload = p_adv - p_task
        ...
        params[name] = (
            p0
            + float(alpha) * delta_task
            + float(eta) * delta_syn
        )
        if include_payload:
            params[name] = params[name] + float(alpha) * delta_payload
...
def main():
    ...
    log_path = os.path.join(output_dir, f"{timestamp}_dtk_bind_pgain_train_log.jsonl")
    config_path = os.path.join(output_dir, f"{timestamp}_dtk_bind_pgain_config.json")
    summary_path = os.path.join(output_dir, f"{timestamp}_dtk_bind_pgain_summary.json")
    ...
    if not os.path.exists(args.clean_task_checkpoint):
        raise FileNotFoundError(args.clean_task_checkpoint)
    clean_task_encoder = load_image_encoder_from_checkpoint(
        args,
        args.clean_task_checkpoint,
        device,
    )
    ...
    print("[DTK-BIND-PGAIN] binding key:", binding_key_stats, flush=True)
    ...
    save_json(
        {
            "method": "KDR-DTK Payload-Specific Gain + Counterfactual Key Binding",
            "stage1": "reuse fixed KDR_DTK trigger",
            "stage2_change": (
                "add payload-specific target-gain supervision while preserving "
                "counterfactual key-removal binding"
            ),
            "payload_gain_definition": (
            ...
            "key_component_gradient": (
                "detached intervention; binding loss cannot directly enlarge the measured key "
                "component within the current optimization step"
            ),
            "args": vars(args),
            "device": str(device),
            "pretrained_path": pretrained_path,
            "init_checkpoint": args.init_checkpoint or pretrained_path,
            "clean_task_checkpoint": args.clean_task_checkpoint,
            "trigger_path": args.trigger_path,
    ...
    print("[DTK-BIND-PGAIN] output_dir:", output_dir, flush=True)
    print("[DTK-BIND-PGAIN] trainable:", trainable_stats, flush=True)
    print("[DTK-BIND-PGAIN] trigger:", args.trigger_path, flush=True)
    print("[DTK-BIND-PGAIN] clean_task:", args.clean_task_checkpoint, flush=True)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            theta_payload_bg_params = build_task_conditioned_params_from_cache(
                image_encoder,
                base_image_encoder,
                clean_task_encoder,
                drift_cache,
            ...
            theta_legacy_bg_params = build_drifted_params_from_cache(
                image_encoder,
                base_image_encoder,
                drift_cache,
                alpha=alpha,
            ...
            key_component = (key_coeff.unsqueeze(1) * binding_key.unsqueeze(0)).detach()
            ...
            with torch.no_grad():
                z_payload_bg = call_with_params(
                    image_encoder,
                    theta_payload_bg_params,
                    bd_images,
                )
                logits_payload_bg = classification_logits(classification_head, z_payload_bg)
                z_legacy_bg = call_with_params(
                    image_encoder,
                    theta_legacy_bg_params,
                    bd_images,
                )
                logits_legacy_bg = classification_logits(classification_head, z_legacy_bg)

            payload_target_gain = (
                logits_atk[:, args.target_cls]
                - logits_payload_bg[:, args.target_cls].detach()
            )
            legacy_residual_target_gain = (
                logits_atk[:, args.target_cls]
                - logits_legacy_bg[:, args.target_cls].detach()
            )
            gain_loss = F.relu(args.gain_eps - payload_target_gain).mean()
            ...
            with torch.no_grad():
                ...
                payload_bg_pred = logits_payload_bg.argmax(dim=1)
                legacy_bg_pred = logits_legacy_bg.argmax(dim=1)
                record = {
                    "epoch": epoch,
                    "batch": batch_idx,
                    ...
                    "payload_target_gain_mean": float(payload_target_gain.mean().item()),
                    "payload_target_gain_min": float(payload_target_gain.min().item()),
                    "legacy_residual_target_gain_mean": float(
                        legacy_residual_target_gain.mean().item()
                    ),
                    ...
                    "payload_bg_target_rate": float(
                        (payload_bg_pred == args.target_cls).float().mean().item()
                    ),
                    "legacy_bg_target_rate": float(
                        (legacy_bg_pred == args.target_cls).float().mean().item()
                    ),
            ...
            if batch_idx % args.log_every == 0:
                print(
                    ...
                    f"bd={record['bd_loss']:.4f} pgain={record['payload_target_gain_mean']:.4f} "
                    f"gain_loss={record['gain_loss']:.4f} bind={record['bind_loss']:.4f} "
        ...
        for key in (
            ...
            "key_coeff_abs_mean",
            "payload_target_gain_mean",
            "payload_target_gain_min",
            "legacy_residual_target_gain_mean",
            "atk_target_rate",
            "key_removed_target_rate",
            "payload_bg_target_rate",
            "legacy_bg_target_rate",
            "clean_acc",
        ):
            epoch_summary[f"avg_{key}"] = mean_record(epoch_records, key)

        append_jsonl({"epoch_summary": epoch_summary}, log_path)
        print("[DTK-BIND-PGAIN] epoch_summary:", epoch_summary, flush=True)
    ...
    summary = {
        ...
        "final_avg_gain_loss": mean_record(last_epoch_records, "gain_loss"),
        "final_avg_payload_target_gain": mean_record(last_epoch_records, "payload_target_gain_mean"),
        "final_avg_payload_target_gain_min": mean_record(last_epoch_records, "payload_target_gain_min"),
        "final_avg_legacy_residual_target_gain": mean_record(
            last_epoch_records,
            "legacy_residual_target_gain_mean",
        ),
    ...
    print("[DTK-BIND-PGAIN] saved checkpoint:", ckpt_path, flush=True)
    print("[DTK-BIND-PGAIN] saved summary:", summary_path, flush=True)
