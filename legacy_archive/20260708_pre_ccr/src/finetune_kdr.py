...
def parse_args():
    parser = argparse.ArgumentParser("KDR-SubMerge synthetic-drift residual editing")
    ...
    parser.add_argument("--method-name", default="KDR")
...
def main():
    ...
    log_path = os.path.join(output_dir, f"{timestamp}_kdr_train_log.jsonl")
    config_path = os.path.join(output_dir, f"{timestamp}_kdr_config.json")
    summary_path = os.path.join(output_dir, f"{timestamp}_kdr_summary.json")
    ...
    print("[KDR] output_dir:", output_dir, flush=True)
    print("[KDR] trainable:", trainable_stats, flush=True)
    print("[KDR] trigger:", args.trigger_path, flush=True)
    ...
    for epoch in range(args.epochs):
        ...
        print("[KDR] epoch_summary:", epoch_summary, flush=True)
    ...
    print("[KDR] saved checkpoint:", ckpt_path, flush=True)
    print("[KDR] saved summary:", summary_path, flush=True)
