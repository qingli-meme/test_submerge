# ============================================================
# KDR-DTK Sender-Calibrated Binding
# ============================================================
...
def parse_args():
    parser = argparse.ArgumentParser(
        "KDR-DTK sender-calibrated counterfactual key binding"
    )
    ...
    parser.add_argument("--method-name", default="KDR_DTK_SCB")
...
def main():
    ...
    log_path = os.path.join(
        output_dir,
        f"{timestamp}_dtk_scb_train_log.jsonl",
    )
    config_path = os.path.join(
        output_dir,
        f"{timestamp}_dtk_scb_config.json",
    )
    summary_path = os.path.join(
        output_dir,
        f"{timestamp}_dtk_scb_summary.json",
    )
    ...
    save_json(
        {
            ...
            "stage1": (
                "reuse the fixed KDR_DTK trigger and the "
                "empirical Stage-1 key direction estimated "
                "from the frozen pretrained sender"
            ),
            "stage2_change": (
                "remove the attack proxy's endogenous key "
                "coordinate, then reconstruct the positive "
                "branch with the frozen Stage-1 sender dose "
                "computed for the same samples"
            ),
            "binding_definition": (
                "the sender-dose positive branch must classify "
                "to the target; after removing the current "
                "attack-proxy key component, the target margin "
                "must be non-positive"
            ),
            "sender_dose_definition": (
                "a0_i = (h_theta0(T(x_i)) - h_theta0(x_i))^T k; "
                "signed per-sample coefficients are computed "
                "from the frozen pretrained encoder"
            ),
            "positive_branch_definition": (
                "h_on = h_atk_triggered - a_atk_i * k + a0_i * k"
            ),
            "full_branch_role": (
                "the unmodified triggered attack-proxy branch "
                "is diagnostic only and is not used by the "
                "backdoor CE or positive-margin loss"
            ),
            "key_component_gradient": (
                "exact differentiable removal of the current "
                "attack-proxy key projection; the sender dose "
                "is frozen and does not receive gradients"
            ),
            "args": vars(args),
            "device": str(device),
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(
            ...
            # ------------------------------------------------
            # Estimate current trigger-induced key component
            # under the exact same attack proxy.
            #
            ...
            # ------------------------------------------------
            # Frozen Stage-1 sender dose for the SAME samples.
            #
            # The sender coefficient is not a global median and
            # is not derived from the trainable attack proxy.
            # For every current batch sample:
            #
            #   a0_i
            #     = (h_theta0(T(x_i)) - h_theta0(x_i))^T k
            #
            # One combined frozen-sender forward is used to
            # avoid redundant clean/triggered passes.
            # ------------------------------------------------

            with torch.no_grad():
                sender_combined = torch.cat(
                    [bd_clean_images, bd_images],
                    dim=0,
                )

                _, sender_combined_act = (
                    capture_regular_layer_activation(
                        base_image_encoder,
                        sender_combined,
                        resolved_key_layer,
                        args.key_pool,
                ...
                sender_key_coeff = torch.matmul(
                    sender_shift,
                    binding_key,
                )

                sender_key_component = (
                    sender_key_coeff.unsqueeze(1)
                    * binding_key.unsqueeze(0)
                )

            # ------------------------------------------------
            # Current attack-proxy key coordinate.
            # ------------------------------------------------
            ...
            # ------------------------------------------------
            # Exact projection removal of the CURRENT
            # endogenous key coordinate:
            #
            #   h_perp
            #     = h_atk_triggered - a_atk_i * k
            #
            # Do not detach key_component.
            # ------------------------------------------------
            ...
            # ------------------------------------------------
            # Sender-calibrated positive branch:
            #
            #   h_on
            #     = h_atk_triggered
            #       - a_atk_i * k
            #       + a0_i * k
            #
            # The current endogenous dose is removed and the
            # frozen Stage-1 sender dose for the SAME sample is
            # reinserted.
            #
            # sender_key_component is frozen because theta0 and
            # k are frozen. The branch therefore trains the
            # receiver to decode the sender-defined signal
            # scale instead of increasing a_atk_i.
            # ------------------------------------------------

            sender_calibrated_delta = (
                sender_key_component
                - key_component
            )
            ...
            # ------------------------------------------------
            # Positive attack supervision is applied ONLY to
            # the sender-calibrated branch.
            #
            # The real full triggered branch is diagnostic only.
            # Keeping CE(full) would preserve the original
            # amplitude shortcut by continuing to reward growth
            # of the endogenous current key coefficient.
            # ------------------------------------------------
            ...
            # ------------------------------------------------
            # Legacy residual target gain is diagnostic only.
            # ------------------------------------------------
            ...
            with torch.no_grad():
                ...
                sender_pred = logits_sender.argmax(
                    dim=1
                )
                removed_pred = (
                ...
                record = {
                    ...
                    "sender_key_gain_mean": float(
                        sender_key_gain.mean().item()
                    ),
                    "sender_key_gain_min": float(
                        sender_key_gain.min().item()
                    ),
                    ...
                    "sender_margin_mean": float(
                        sender_margin.mean().item()
                    ),
                    "sender_margin_min": float(
                        sender_margin.min().item()
                    ),
                    ...
                    "key_coeff_abs_mean": float(
                        current_abs.mean().item()
                    ),
                    "sender_key_coeff_mean": float(
                        sender_key_coeff.mean().item()
                    ),
                    "sender_key_coeff_abs_mean": float(
                        sender_abs.mean().item()
                    ),
                    "sender_key_coeff_positive_rate": float(
                        (
                            sender_key_coeff > 0
                        )
                        .float()
                    ...
                    "current_sender_ratio_mean": float(
                        current_sender_ratio.mean().item()
                    ),
                    "current_sender_ratio_median": float(
                        current_sender_ratio.median().item()
                    ),
            ...
            if batch_idx % args.log_every == 0:
                print(
                    ...
                    f"sender_gain={record['sender_key_gain_mean']:.4f} "
                    f"M(full)={record['full_margin_mean']:.4f} "
                    f"M(sender)={record['sender_margin_mean']:.4f} "
                    f"M(off)={record['key_removed_margin_mean']:.4f} "
                    f"full={record['atk_target_rate']:.3f} "
                    f"sender={record['sender_target_rate']:.3f} "
                    f"off={record['key_removed_target_rate']:.3f} "
                    f"dose_ratio={record['current_sender_ratio_median']:.3f}",
                    flush=True,
                )
        ...
        for key in (
            ...
            "key_coeff_abs_mean",
            "sender_key_coeff_mean",
            "sender_key_coeff_abs_mean",
            "sender_key_coeff_positive_rate",
            "current_sender_ratio_mean",
            "current_sender_ratio_median",
    ...
    summary = {
        ...
        "final_avg_sender_key_gain": mean_record(
            last_epoch_records,
            "sender_key_gain_mean",
        ),
        ...
        "final_avg_sender_margin": mean_record(
            last_epoch_records,
            "sender_margin_mean",
        ),
        ...
        "final_avg_sender_target_rate": mean_record(
            last_epoch_records,
            "sender_target_rate",
        ),
        ...
        "final_avg_current_key_coeff_abs": mean_record(
            last_epoch_records,
            "key_coeff_abs_mean",
        ),
        "final_avg_sender_key_coeff_abs": mean_record(
            last_epoch_records,
            "sender_key_coeff_abs_mean",
        ),
        "final_avg_sender_key_coeff_positive_rate": mean_record(
            last_epoch_records,
            "sender_key_coeff_positive_rate",
        ),
        "final_avg_current_sender_ratio": mean_record(
            last_epoch_records,
            "current_sender_ratio_median",
        ),
    ...
    print(
        "[DTK-SCB] saved checkpoint:",
        ckpt_path,
        flush=True,
    )
    print(
        "[DTK-SCB] saved summary:",
        summary_path,
        flush=True,
    )
