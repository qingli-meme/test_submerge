...
alt_root="./tmp/checkpoints_bmr_cars_e35_lr1e5"
...
results_dir="./results/bmr_multitask_cars_e35_lr1e5"
ada_dir="./ada/${model}"
ada_path="${ada_dir}/KDR_DTK_BMR_${task}_Tgt_${target_cls}_L_${patch_size}_Epoch_${ada_epochs}.pt"
ada_lr_path="${ada_dir}/KDR_DTK_BMR_${task}_E35_LR1e5_Tgt_${target_cls}_L_${patch_size}_Epoch_${ada_epochs}.pt"
backup_path="${ada_path}.pre_e35_lr1e5.$(date +%Y%m%d_%H%M%S).bak"
ts="$(date +%Y%m%d_%H%M%S)"
log="./logs/bmr_cars_e35_lr1e5_gpu${gpu_id}_${ts}.log"
...
{
  echo "============================================================"
  echo "[Cars E35 lr1e-5] gpu=${gpu_id} start=$(date --iso-8601=seconds)"
...
if [ ! -f "${alt_ckpt}" ]; then
  ...
  echo "[Cars E35 lr1e-5] reuse checkpoint ${alt_ckpt}" | tee -a "${log}"
fi
...
if [ ! -f "${ada_lr_path}" ]; then
  if [ -f "${ada_path}" ]; then
    ...
    echo "[Cars E35 lr1e-5] backed up ${ada_path} -> ${backup_path}" | tee -a "${log}"
  fi
  ...
  echo "[Cars E35 lr1e-5] saved AdaMerging ${ada_lr_path}" | tee -a "${log}"
  if [ -f "${backup_path}" ]; then
    mv "${backup_path}" "${ada_path}"
    echo "[Cars E35 lr1e-5] restored previous ${ada_path}" | tee -a "${log}"
  fi
else
  echo "[Cars E35 lr1e-5] reuse AdaMerging ${ada_lr_path}" | tee -a "${log}"
fi

echo "[Cars E35 lr1e-5] done $(date --iso-8601=seconds)" | tee -a "${log}"
