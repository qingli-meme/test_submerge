...
elif attack_type in ('KDR', 'KDR_COV', 'KDR_GC', 'KDR_TCB', 'KDR_TVD', 'KDR_TDK', 'KDR_TDK_BMR', 'KDR_DTK', 'KDR_DTK_BAB', 'KDR_DTK_BIND', 'KDR_DTK_BIND_EXACT', 'KDR_DTK_SCB', 'KDR_DTK_FSB', 'KDR_DTK_CCR', 'KDR_DTK_BMR', 'KDR_DTK_BIND_PGAIN'):
    if attack_type in ('KDR_GC', 'KDR_TCB', 'KDR_TVD'):
        trigger_type = 'KDR'
    elif attack_type in ('KDR_TDK_BMR',):
        trigger_type = 'KDR_TDK'
    elif attack_type in ('KDR_DTK_BIND', 'KDR_DTK_BIND_EXACT', 'KDR_DTK_SCB', 'KDR_DTK_FSB', 'KDR_DTK_CCR', 'KDR_DTK_BMR', 'KDR_DTK_BIND_PGAIN'):
        trigger_type = 'KDR_DTK'
    else:
        trigger_type = attack_type
    trigger_path = os.path.join(args.trigger_dir, f'{trigger_type}_{adversary_task}_Tgt_{target_cls}_L_{patch_size}.npy')
    trigger = np.load(trigger_path)
    trigger = torch.from_numpy(trigger)
else: # Ours
...
for dataset_name in exam_datasets:
    ...
    if dataset_name==adversary_task and attack_type != 'Clean':
        if attack_type in ('KDR', 'KDR_COV', 'KDR_GC', 'KDR_TCB', 'KDR_TVD', 'KDR_TDK', 'KDR_TDK_BMR', 'KDR_DTK', 'KDR_DTK_BAB', 'KDR_DTK_BIND', 'KDR_DTK_BIND_EXACT', 'KDR_DTK_SCB', 'KDR_DTK_FSB', 'KDR_DTK_CCR', 'KDR_DTK_BMR', 'KDR_DTK_BIND_PGAIN'):
            ckpt_name = os.path.join(args.save, dataset_name+f'_{attack_type}_{adversary_task}_Tgt_{target_cls}_L_{patch_size}', 'finetuned.pt')
        else:
...
for epoch in tqdm.tqdm(range(epochs)):
    ...
    if save_flag and ep==epochs:   
        ...
        if attack_type=='Clean':
            ...
            torch.save(adamerging_mtl_model.lambdas_raw, os.path.join(adamerging_dir, f"{attack_type}_{adversary_task}_Tgt_{target_cls}_L_{patch_size}_Epoch_{ep}.pt"))
        else:
