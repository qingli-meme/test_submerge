# ============================================================
# Covariance-Aligned Edit-Key Patch Construction
# ============================================================
# KDR_COV only modifies KDR Stage-1 trigger construction.
#
# Difference from optimize_edit_key_patch.py:
#   Original KDR learns a stable representation key.
#   KDR_COV additionally encourages the trigger-induced key shift
#   to lie in high-energy clean activation covariance directions.
#
# No target label is used.
# No target CE is used.
# No target logit is maximized.
...
def parse_args():
    p = argparse.ArgumentParser("Optimize Covariance-Aligned Edit-Key Patch")
    ...
    p.add_argument("--cov-weight", type=float, default=0.5)
    p.add_argument("--cov-rank", type=int, default=32)
    ...
    p.add_argument("--save-path", default="./trigger/ViT-B-32/KDR_COV_CIFAR100_Tgt_1_L_22.npy")
    p.add_argument("--out-dir", default="./analysis/kdr_cov_key_patch")
    p.add_argument("--seed", type=int, default=2026)
    p.add_argument("--log-every", type=int, default=20)
    return p.parse_args()


def clean_cov_top_subspace(clean_feat: torch.Tensor, rank: int):
    with torch.no_grad():
        x = clean_feat.detach().float()
        x = x - x.mean(dim=0, keepdim=True)
        b, d = x.shape
        r = max(1, min(int(rank), b - 1, d))
        ...
        try:
            ...
            v = eigvecs[:, idx].contiguous()

        return F.normalize(v, dim=0).detach()


def covariance_alignment_loss(shift: torch.Tensor, clean_feat: torch.Tensor, rank: int):
    v = clean_cov_top_subspace(clean_feat, rank=rank)
    shift_norm = F.normalize(shift, dim=-1)
    proj = shift_norm.float() @ v.to(device=shift.device, dtype=torch.float32)
    cov_energy = proj.pow(2).sum(dim=1)
    loss = -cov_energy.mean()
    return loss, cov_energy.detach()
...
def main():
    ...
    log_path = os.path.join(args.out_dir, f"{timestamp}_cov_edit_key_patch_log.jsonl")
    config_path = os.path.join(args.out_dir, f"{timestamp}_cov_edit_key_patch_config.json")
    save_json(
        {
            "method": "Covariance-Aligned Edit-Key Patch",
            "target_free": True,
            "args": vars(args),
            "log_path": log_path,
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            cov_loss, cov_energy = covariance_alignment_loss(
                shift=shift,
            ...
            record = {
                ...
                "mean_cov_energy": float(cov_energy.mean().item()),
            ...
            if batch_idx % args.log_every == 0:
                print(
                    ...
                    f"cov={record['mean_cov_energy']:.4f} "
    ...
    summary_path = os.path.join(args.out_dir, f"{timestamp}_cov_edit_key_patch_summary.json")
    save_json(
        {
            "method": "Covariance-Aligned Edit-Key Patch",
            "save_path": args.save_path,
    ...
    print(f"[CovKey] saved trigger: {args.save_path}")
    print(f"[CovKey] saved summary: {summary_path}")
