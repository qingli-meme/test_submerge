...
mkdir -p \
  ./analysis/kdr_dtk_ccr_final_state_readout \
  ./logs

ts="$(date +%Y%m%d_%H%M%S)"
log_path="./logs/kdr_dtk_ccr_final_state_readout_${ts}.log"
...
python3 \
  ...
  --eps 1e-8 \
  --seed 20260705 \
  --out-dir ./analysis/kdr_dtk_ccr_final_state_readout \
  2>&1 | tee "${log_path}"

echo "[KDR-DTK-CCR final-state readout] log: ${log_path}"